...
public class StringUtils{
	...
	public static boolean isNullOrEmpty(BigDecimal locAmount) {
		if (locAmount != null) {
			return false;
		}
		return true;
	 ...
	 * @return 返回参照值
	 */
	public static String replaceRef(String strs) {
		...
		if (strs.indexOf(",") > 0) {
			return getMultipleStrs(strs, substring);
		}

		return getSingleStr(strs);

	}

	private static String getSingleStr(String strs) {

		return strs.replace("\"", "");
	}

	private static String getMultipleStrs(String strs, String substring) {
		...
		for (String str : splits) {
			substring = substring + str.replace("\"", "");

		}
		return substring;
	}

	public static List<String> getStrs(String strs) {
     ...
     * @return
     */
    public static String parseSqlByParameter(String sql, Map<String,String> parameter) {
        List<String> excludes = new ArrayList<String>();
        Pattern p = Pattern.compile("(/~[^~]*)?[:@](\\w+)[^\\w]([^~@/]*~/)?");
        Matcher  matcher = p.matcher(sql);
        while(matcher.find()) {
            if(parameter != null && parameter.containsKey(matcher.group(2))) {  //如果存在参数
                String value = String.valueOf(parameter.get(matcher.group(2)));
                if(matcher.group(1).toLowerCase().indexOf("like") != -1){
                    value = "'%"+value+"%'";
                }else if(matcher.group(1).toLowerCase().indexOf("in") != -1){
                    value = value+"";
                }else{
                    value = "'"+value+"'";
                }
                sql = sql.replaceAll(":"+matcher.group(2), value);
            }else if(matcher.group(0).startsWith("/~")){  //如果不存在，但是以/~开头，表示可选项
                excludes.add(matcher.group(2));//动态去掉不需要的表达式
            }
        }
        if(excludes.size() > 0) {//过滤掉不需要的表达式
            for(String exclude: excludes) {
                sql = sql.replaceAll("/~[^~]*[:@]" + exclude + "[^~\\/]*~/", "");
            }
        }
        //把特殊字符(/~ ~/)去掉，并把变量替换为?号
        sql = sql.replaceAll("/~([^~]*)[:@]\\w+([^\\w~]*[^~]*)~/", "$1?$2");
        sql = sql.replaceAll("/~|~/", "");
        return sql;
    }
